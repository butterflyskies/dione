...
pub fn classify_linear(
    ...
    is_thread: bool,
    thread_branch_id: Option<u64>,
    candidates: &[(u64, FeatureVector)],
) -> ClassificationResult {
    // Thread shortcut: Discord threads skip scoring.
    if is_thread {
        if let Some(branch_id) = thread_branch_id {
    ...
    for &(branch_id, ref fv) in candidates {
...
///
/// Returns 1.0 for elapsed=0, ~0.5 at one half-life, approaching 0 as
/// elapsed grows.
...
mod tests {
    ...
    /// A trivial embedder that returns a fixed vector based on the first char,
    /// useful for testing without a real model.
    struct MockEmbedder;

    impl Embedder for MockEmbedder {
        fn embed(&self, text: &str) -> Vec<f32> {
            // Produce a 3-dimensional embedding based on the first char's
            // codepoint, normalized to unit length.
    ...
    fn temporal_decay_at_half_life() {
        ...
        assert!(
            (val - 0.5).abs() < 1e-9,
            "at half-life should be ~0.5, got {val}"
        );
    }
    ...
    fn temporal_decay_large_elapsed() {
        ...
        assert!(val < 0.01, "10 half-lives should be near zero, got {val}");
    }

    #[test]
    fn temporal_decay_negative_elapsed() {
        assert_eq!(
            temporal_decay(-10.0, 300.0),
            1.0,
            "negative elapsed clamps to 1.0"
        );
    }
    ...
    fn temporal_decay_zero_half_life() {
        assert_eq!(
            temporal_decay(10.0, 0.0),
            0.0,
            "zero half-life yields instant decay"
        );
    }
    ...
    fn participant_overlap_partial() {
        ...
        assert!(
            ...
            "one of three matches: expected {expected}, got {ratio}"
        );
    }
    ...
    fn participant_overlap_none() {
        ...
        assert!(ratio.abs() < 1e-9, "no overlap: {ratio}");
    }

    #[test]
    fn participant_overlap_empty_message() {
        let ratio = participant_overlap_ratio(&[], &[1, 2]);
        assert_eq!(ratio, 0.0, "empty message participants yields 0.0");
    }
    ...
    fn linear_score_reply_only() {
        ...
        assert!(
            (score - 1.0).abs() < 1e-9,
            "reply-only should score 1.0, got {score}"
        );
    }
    ...
    fn linear_score_no_signals() {
        ...
        assert!(
            fv.score_linear(&ScoringWeights::default()).is_none(),
            "no signals should return None"
        );
    }

    #[test]
    fn linear_score_conditional_normalization() {
        // When reply_chain is absent, its 0.8 weight should be excluded from
        // the denominator.
        let fv_with_reply = FeatureVector {
        ...
        let fv_without_reply = FeatureVector {
            reply_chain: None,
            topic_similarity: Some(0.5),
            participant_overlap: None,
            temporal_proximity: None,
        ...
        let score_with = fv_with_reply.score_linear(&w).unwrap();
        let score_without = fv_without_reply.score_linear(&w).unwrap();

        // With reply: (0.0*0.8 + 0.5*0.4) / (0.8+0.4) = 0.2/1.2 = 0.1667
        // Without reply: (0.5*0.4) / 0.4 = 0.5
        assert!(
            (score_with - 0.2 / 1.2).abs() < 1e-9,
            "with zero reply: expected {}, got {score_with}",
            0.2 / 1.2
        );
        assert!(
            (score_without - 0.5).abs() < 1e-9,
            "without reply: expected 0.5, got {score_without}"
        );
        assert!(
            score_without > score_with,
            "absent signal should produce higher score than zero signal"
        );
    }
    ...
    fn cascade_reply_deterministic() {
        ...
        assert!(
            (score - 1.0).abs() < 1e-9,
            "reply chain hit should be 1.0, got {score}"
        );
    }
    ...
    fn cascade_topic_and_temporal() {
        ...
        // Core: (0.8*0.4 + 0.6*0.3) / (0.4+0.3) = (0.32+0.18)/0.7 = 0.5/0.7 ≈ 0.714
    ...
    #[test]
    fn cascade_with_tiebreaker_bonus() {
        let fv = FeatureVector {
            reply_chain: None,
            topic_similarity: Some(0.5),
        ...
        let w = ScoringWeights::default();
        let score = fv.score_cascade(&w).expect("should produce a score");
        // Core: (0.5*0.4 + 0.5*0.3) / (0.4+0.3) = 0.35/0.7 = 0.5
        // Bonus: 1.0*(0.3/2.0) + 1.0*(0.2/2.0) = 0.15 + 0.1 = 0.25
        // Total: 0.5 + 0.25 = 0.75
        let expected = 0.5 + 1.0 * (0.3 / 2.0) + 1.0 * (0.2 / 2.0);
        assert!(
            (score - expected).abs() < 1e-9,
            "expected {expected}, got {score}"
        );
    ...
    fn cascade_no_signals() {
        ...
        assert!(
            fv.score_cascade(&ScoringWeights::default()).is_none(),
            "no signals should return None"
        );
    }

    #[test]
    fn cascade_tiebreaker_only() {
        let fv = FeatureVector {
            reply_chain: None,
            topic_similarity: None,
            participant_overlap: Some(1.0),
        ...
        let w = ScoringWeights::default();
        let score = fv.score_cascade(&w).expect("tiebreaker-only should score");
        // Bonus: 1.0*(0.3/2.0) + 1.0*(0.2/2.0) = 0.25
        let expected = 1.0 * (0.3 / 2.0) + 1.0 * (0.2 / 2.0);
        assert!(
            (score - expected).abs() < 1e-9,
            "expected {expected}, got {score}"
        );
    ...
    // ── Classification: thread shortcut ──────────────────────────────────────

    #[test]
    fn thread_shortcut_linear() {
        let config = ScoringConfig::default();
        let result = classify_linear(&config, true, Some(42), &[]);
        assert_eq!(
            result,
            ...
            "threads should be deterministically assigned"
        );
    }
    ...
    fn thread_shortcut_cascade() {
        let config = ScoringConfig::default();
        let result = classify_cascade(&config, true, Some(42), &[]);
        assert_eq!(
            result,
            ClassificationResult::Assigned(BranchScore {
                branch_id: 42,
    ...
    #[test]
    fn thread_without_branch_id_falls_through() {
        let config = ScoringConfig::default();
        let result = classify_linear(&config, true, None, &[]);
        assert_eq!(result, ClassificationResult::NewBranch);
    }

    // ── Classification: new branch ───────────────────────────────────────────
    ...
    fn new_branch_when_below_threshold() {
        ...
        let candidates = vec![(1, fv)];
        assert_eq!(
            classify_linear(&config, false, None, &candidates),
            ClassificationResult::NewBranch,
            "score below threshold should create new branch"
        );
    }

    // ── Classification: reply-to matching ────────────────────────────────────
    ...
    fn reply_to_assigns_to_correct_branch() {
        ...
        let candidates = vec![(10, fv_other), (20, fv_match)];

        let result = classify_linear(&config, false, None, &candidates);
        match result {
            ClassificationResult::Assigned(bs) => {
                assert_eq!(bs.branch_id, 20, "should assign to branch with reply match");
                assert!(
                    (bs.score - 1.0).abs() < 1e-9,
                    "reply-only linear should score 1.0"
                );
            }
    ...
    // ── Classification: topic-only matching ──────────────────────────────────

    #[test]
    fn topic_only_assigns_above_threshold() {
        let config = ScoringConfig::default();
        let fv = FeatureVector {
            reply_chain: None,
        ...
        let candidates = vec![(5, fv)];

        let result = classify_linear(&config, false, None, &candidates);
        match result {
            ClassificationResult::Assigned(bs) => {
                assert_eq!(bs.branch_id, 5);
                assert!(
                    (bs.score - 0.9).abs() < 1e-9,
                    "topic-only linear: expected 0.9, got {}",
                    bs.score
                );
                assert_eq!(bs.confidence, AssignmentConfidence::High);
            }
            _ => panic!("expected Assigned, got {result:?}"),
        }
    }

    // ── Both variants agree on clear cases ───────────────────────────────────
    ...
    fn variants_agree_on_strong_reply() {
        ...
        match (&linear, &cascade) {
            ...
            _ => panic!("expected both Assigned, got linear={linear:?} cascade={cascade:?}"),
        }
    }
    ...
    fn custom_threshold() {
        ...
        let fv = FeatureVector {
            reply_chain: None,
            topic_similarity: Some(0.4),
            participant_overlap: None,
            temporal_proximity: None,
            mention_match: None,
        };
        let candidates = vec![(7, fv)];

        let result = classify_linear(&config, false, None, &candidates);
        match result {
            ClassificationResult::Assigned(bs) => {
                assert_eq!(bs.branch_id, 7);
            }
            _ => panic!("score 0.4 should be above threshold 0.3"),
        }
    }
    ...
    fn selects_highest_scoring_branch() {
        ...
        let candidates = vec![(1, fv_low), (2, fv_high)];

        let result = classify_linear(&config, false, None, &candidates);
        match result {
            ClassificationResult::Assigned(bs) => {
                assert_eq!(bs.branch_id, 2, "should pick the higher-scoring branch");
            }
    ...
    #[test]
    fn linear_score_clamps_to_unit_range() {
        // Construct a scenario where raw weighted sum > weight total cannot
        // happen with valid inputs (signals capped at 1.0), but verify
        // clamping works at the boundary.
        let fv = FeatureVector {
            reply_chain: Some(1.0),
            topic_similarity: Some(1.0),
            participant_overlap: Some(1.0),
            temporal_proximity: Some(1.0),
            mention_match: Some(1.0),
        };
        let score = fv.score_linear(&ScoringWeights::default()).unwrap();
        assert!(
            score <= 1.0 && score >= 0.0,
        ...
        assert!(
            (score - 1.0).abs() < 1e-9,
            "all-ones should score 1.0, got {score}"
        );
    }
    ...
    #[test]
    fn mock_embedder_produces_unit_vectors() {
        let e = MockEmbedder;
        let v = e.embed("test");
        let mag: f32 = v.iter().map(|x| x * x).sum::<f32>().sqrt();
        assert!(
            (mag - 1.0).abs() < 1e-5,
            "embedding should be unit length, got magnitude {mag}"
        );
    }

    #[test]
    fn mock_embedder_same_prefix_is_similar() {
        let e = MockEmbedder;
        let a = e.embed("apple");
        let b = e.embed("avocado");
        let sim = e.cosine_similarity(&a, &b);
        assert!(
            (sim - 1.0).abs() < 1e-5,
            "same first char means identical mock vectors, sim should be ~1.0, got {sim}"
        );
    }

    #[test]
    fn mock_embedder_different_prefix_still_similar_in_mock() {
        // The mock embedder produces vectors along the same direction
        // (proportional to [c, c/2, c/4]), so all non-zero chars will have
        // sim ≈ 1.0. This is expected for the mock — real tests against a
        // real model would show variation.
        let e = MockEmbedder;
        let a = e.embed("apple");
        let b = e.embed("banana");
        let sim = e.cosine_similarity(&a, &b);
        assert!(
            sim > 0.99,
            "mock embedder vectors are proportional, sim should be ~1.0, got {sim}"
        );
    }
